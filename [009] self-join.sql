...
-- #4
# Run the query and notice the two services that link these stops have a count 
# of 2. Add a HAVING clause to restrict the output to these two routes.
...
ORDER BY a.num, c.num

-- alternative answer
SELECT DISTINCT S.num, S.company, stops.name ,  E.num,  E.company
FROM 
(SELECT a.company, a.num, b.stop FROM route a JOIN route b 
ON (a.company = b.company AND a.num = b.num)
WHERE a.stop = (SELECT id FROM stops WHERE name = 'Craiglockhart')) AS S
JOIN 
(SELECT a.company, a.num, b.stop FROM route a JOIN route b 
ON (a.company=b.company AND a.num=b.num) 
WHERE a.stop= (SELECT id FROM stops WHERE name = 'Sighthill')) AS E 
ON (S.stop = E.stop)
